...
from datasets.datasets_dfc_digit import DFC2020
...
def parse_option():
    ...
    parser.add_argument('--save', type=str, default='./Siamese', help='path to save linear classifier')
...
class Trainer:
    def __init__(self, args, model, optimizer, criterion, lr_scheduler, device):
    ...
    def train(self, train_loader):
        ...
        for epoch_counter in range(self.max_epochs):
            for idx, batch in enumerate(train_loader):
                ...
                target = batch['label']
                bsz = image.size(0)
                loss, acc1, acc5 = self.update(image, target)
            ...
            print('Epoch: {} \tTraining Loss: {:.6f} \tACC: {:.3f}'.format(epoch_counter, losses.avg, top1.avg))
            # save checkpoints
            if (epoch_counter + 1) % 100 == 0:
                self.save_model(os.path.join(self.savepath, 'Siamese_epoch_{epoch}_{loss}.pth'.format(epoch=epoch_counter, loss=losses.avg)))
            torch.cuda.empty_cache()

    def update(self, image, target):
        '''
        :param image:
        :param target:
        :return:
        ...
        if 'Affine' in aug_type:
            ...
            param.append(RandomAffine_params(1.0, 0.0, random.uniform(-0.2, 0.2), random.uniform(-0.2, 0.2),
                                             self.scl_sz, None, image.shape[0], image.shape[-2:], self.device,
                                             image.dtype))
        '''
...
def main():
    ...
    # set the model
    model = Siamese(n_inputs=n_inputs, n_clasess=n_classes).to(device)
    ...
    trainer = Trainer(args, model, optimizer, criterion, lr_scheduler, device)
