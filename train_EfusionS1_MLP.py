...
from utils.util import adjust_learning_rate, AverageMeter, accuracy, seed_torch, visualization, visualizationS1
...
def parse_option():
    ...
    parser.add_argument('--save', type=str, default='./mlp_EfusionS1_1000', help='path to save linear classifier')
    parser.add_argument('--save_freq', type=int, default=50, help='number of training epochs')
    parser.add_argument('--preview_dir', type=str, default='./preview_EfusionS1',
...
def unet_encoder_factory(args, pretrained=True):
    ...
    """
    # build model
    model = E_Fusion(width=1, in_channel=2, in_dim=args.in_dim, feat_dim=args.feat_dim).to(args.device)
    if pretrained:
        # load pre-trained model
        print('==> loading pre-trained model')
        ckpt = torch.load('./save/EfusionS1/twins_epoch_699_11.524518966674805.pth')
...
def train(epoch, train_loader, online_network, classifier, criterion, optimizer, args):
    ...
    for idx, (batch) in enumerate(train_loader):
        ...
        _, image = torch.split(image, [4, 2], dim=1)
        if args.use_gpu:
...
def validate(val_loader, online_network, classifier, criterion, args):
    ...
    with torch.no_grad():
        for idx, (batch) in enumerate(val_loader):
            ...
            if args.visual:
                visualizationS1(prediction, target, batch['id'], image, args)
