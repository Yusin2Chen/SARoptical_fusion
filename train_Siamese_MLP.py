...
from utils.datasets_dfc import DFC2020
from networks.linear_eva import ResMlp
from networks.Siamese import Siamese
...
def parse_option():
    ...
    parser.add_argument('--save', type=str, default='./mlp_Siamese1000', help='path to save linear classifier')
    parser.add_argument('--save_freq', type=int, default=50, help='number of training epochs')
    parser.add_argument('--preview_dir', type=str, default='./preview_Siamese',
...
def cmc_encoder_factory(args, pretrained=True):
    ...
    """
    # build model
    model = Siamese(n_inputs=args.n_inputs, n_clasess=args.n_classes)
    if pretrained:
        # load pre-trained model
        print('==> loading pre-trained model')
        ckpt = torch.load('./save/Siamese/Siamese_epoch_699_0.3846477650744574.pth')
...
def train(epoch, train_loader, pretra_network, classifier, criterion, optimizer, args):
    """
    one epoch training
    """
    ...
    for idx, (batch) in enumerate(train_loader):
        ...
        # ===================forward=====================
        with torch.no_grad():
            g_SAR1, g_OPT1 = pretra_network(image, mode=0)
        prediction = classifier(g_SAR1, g_OPT1)
...
def validate(val_loader, pretra_network, classifier, criterion, args):
    """
    evaluation
    """
...
def main():
    ...
    # set the model
    pretra_network = cmc_encoder_factory(args, pretrained=True).to(device)
    # predictor network
    InChannels = (64 + 64 + 128 + 256) * 1
    classifier = ResMlp(InChannels, 32, n_classes).to(device)
    args.visual = True
    if args.valid:
        try:
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if not args.valid:
            ...
            train(epoch, train_loader, pretra_network, classifier, criterion, optimizer, args)
            if epoch % 50 == 0:
                validate(valid_loader, pretra_network, classifier, criterion, args)
        else:
            validate(valid_loader, pretra_network, classifier, criterion, args)
            break
