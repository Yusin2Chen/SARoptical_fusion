...
class RandomCrop(object):
    ...
    def __call__(self, sample, unlabeld=False, superpixel=False):
        ...
        if unlabeld:
            ...
            lc = np.argmax(np.bincount(lc.reshape(lc.size)))
    ...
    def __init__(self,
        ...
        for folder in sample_dirs:
            ...
            for s2_loc in tqdm(s2_locations, desc="[Load]"):
                ...
                self.samples.append({"lc": lc_loc, "s1": s1_loc, "s2": s2_loc, "id": os.path.basename(lc_loc)})
...
if __name__ == "__main__":
    ...
    ds = DFC2020(data_dir, subset="train", use_s1=False, use_s2hr=True, use_s2mr=False, no_savanna=True, unlabeled=False,
                 train_index='train_3614.npy')
    refer = np.load('train_3614.npy')
    '''
    valid = np.load('vali2000.npy')
    vali_list = []
    classes = []
    ...
    for i in range(3614):
        ...
        if s["id"] in id_list:
            classes.append(s["id"])
            vali_list.append(refer[i])
        #print("imput label:", s["label"])
    print(vali_list)
    vali_list = np.array(vali_list, dtype=np.int32)
    vali_list = np.hstack((vali_list, valid))
    print(vali_list)
    new_valid = vali_list[0:2000]
    np.save('newvali2000.npy', new_valid)
    '''
    vali_list = []
    classes = []
    for i in range(3614):
        ...
        if len(classes)==8:
            ...
            classes = []
    print(vali_list)
    vali_list = np.array(vali_list, dtype=np.int32)[0:16]
    np.save('train_16.npy', vali_list)
