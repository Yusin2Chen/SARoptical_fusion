...
def parse_option():
    ...
    parser.add_argument('--save', type=str, default='./EfusionS1', help='path to save linear classifier')
...
def main():
    ...
    # set the model
    online_network = E_Fusion(width=1, in_channel=2, in_dim=args.in_dim, feat_dim=args.feat_dim).to(device)
    ## load pre-trained model if defined
    if args.resume:
        try:
            print('loading pretrained models')
            checkpoints_folder = os.path.join('.', 'save/EfusionS1')

            # load pre-trained parameters
            load_params = torch.load(
                os.path.join(os.path.join(checkpoints_folder, 'twins_epoch_599_10.689021110534668.pth')),
                map_location=device)
