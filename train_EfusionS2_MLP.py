...
def parse_option():
    ...
    parser.add_argument('--save', type=str, default='./mlp_EfusionS2_1000', help='path to save linear classifier')
    parser.add_argument('--save_freq', type=int, default=50, help='number of training epochs')
    parser.add_argument('--preview_dir', type=str, default='./preview_EfusionS2',
...
def unet_encoder_factory(args, pretrained=True):
    ...
    """
    # build model
    model = E_Fusion(width=1, in_channel=4, in_dim=args.in_dim, feat_dim=args.feat_dim).to(args.device)
    if pretrained:
        # load pre-trained model
        print('==> loading pre-trained model')
        ckpt = torch.load('./save/EfusionS2/twins_epoch_699_10.363265991210938.pth')
