...
from networks.propnets import M_Fusion
...
def parse_option():
    ...
    parser.add_argument('--save', type=str, default='./Mfusion_ori2all', help='path to save linear classifier')
...
class Trainer:
    def __init__(self, args, online_network, optimizer, criterion, scheduler, device):
        ...
        self.args = args
        DEFAULT_AUG = nn.Sequential(augmentation.RandomGaussianBlur((5, 5), (0.1, 2.0), p=0.5),
                                    augmentation.RandomGaussianNoise(mean=0.0, std=1.0, p=0.5),
                                    augmentation.RandomErasing((.05, .3), (0.8, 1.2), p=0.5))
        #DEFAULT_AUG = augmentation.RandomErasing((.1, .3), (0.8, 1.2), p=0.5)
        augment_fn = None
        ...
        self.aug_AFF = RandomAffine(p=1, theta=0, h_trans=random.uniform(-0.2, 0.2), v_trans=random.uniform(-0.2, 0.2), scale=None,
                                    shear=None, interpolation='nearest')
        #self.aug_AFF = RandomAffine(p=1, theta=self.rot_agl, h_trans=0.0, v_trans=0.0, scale=self.scl_sz,
        #                            shear=self.shear, interpolation='nearest')
    ...
    def train(self, train_loader):
        ...
        for epoch_counter in range(self.max_epochs):
            ...
            for idx, batch in enumerate(train_loader):
                if self.lr_warmup < 25:
                    lr_scale = (self.lr_warmup + 1) / 25
                ...
                if self.lr_step == "cos" and self.lr_warmup >= 25:
    ...
    def update(self, image, segmt):
        ...
        l_feature1 = self.get_spix_data(batch_segm_2, one_mask, l_feature1)
        l_feature2 = self.get_spix_data(batch_segm_2, one_mask, l_feature2)

        # sampling list
        #if len(l_feature1) > 50:
        #    feat_sample = 50
        #    feat_list = random.sample(range(len(l_feature1)), feat_sample)
        #    l_feature1 = l_feature1[feat_list, :]
        #    l_feature2 = l_feature2[feat_list, :]
        # pixel loss
    ...
    def get_spix_data(self, batch_segm_2, one_mask, inFeats):
        bs, C, H, W = inFeats.shape
        ...
        self.outFeats = torch.zeros((len(values_idx), C)).to(self.device)
        unique = torch.unique(values_idx, sorted=False, return_inverse=False, dim=0)

        for i in unique:
            s0 = batch_segm_2 == i
            s0_idx = values_idx == i
            # print(i, s0_idx.sum(), s0.sum(axis=1).sum(axis=1))
            spix_idx = s0.sum(axis=1).sum(axis=1) == 0
            # spix_idx_s, spix_idx_idx = spix_idx.sort(descending=True)
            # sspix_idx = spix_idx_idx[0:s0_idx.sum()]
            ex_dim_s0 = s0[:, None, :, :]
            mask_nums = s0.sum(axis=1).sum(axis=1)
            mask_nums[mask_nums == 0] = 1
            mask_nums = mask_nums[:, None]
            masked = ex_dim_s0 * inFeats
            sum_sup_feats = masked.sum(axis=2).sum(axis=2)
            avg_sup_feats = sum_sup_feats / mask_nums
            # print(s0_idx.shape, avg_sup_feats.shape)
            self.outFeats[s0_idx, :] = avg_sup_feats[~spix_idx, :]
...
def main():
    ...
    # set the model
    online_network = M_Fusion(width=1, in_dim=args.in_dim, feat_dim=args.feat_dim).to(device)
    ## load pre-trained model if defined
    if args.resume:
        try:
            print('loading pretrained models')
            #checkpoints_folder = os.path.join('.', 'save/Mfusion_encoder')
            checkpoints_folder = os.path.join('.', 'save/Mfusion_ori2all')
            # load pre-trained parameters
            #load_params = torch.load(os.path.join(os.path.join(checkpoints_folder, 'twins_epoch_199_10.929302096366882.pth')),
            #                         map_location=device)
            load_params = torch.load(os.path.join(os.path.join(checkpoints_folder, 'twins_epoch_999_14.964471817016602.pth')),
                map_location=device)
    ...
    # target encoder
    criterion = HardNegtive_loss(estimator='hard')
    optimizer = torch.optim.Adam(online_network.parameters(), lr=3e-4, weight_decay=1e-4)
    scheduler = get_scheduler(optimizer, args)
    args.lr_warmup_val = 0 if args.lr_warmup else 25
