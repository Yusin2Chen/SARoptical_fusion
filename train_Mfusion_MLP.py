...
def parse_option():
    ...
    parser.add_argument('--save', type=str, default='./mlp_Mfusion1000', help='path to save linear classifier')
    parser.add_argument('--save_freq', type=int, default=50, help='number of training epochs')
    parser.add_argument('--preview_dir', type=str, default='./preview_Mfusion',
...
def unet_encoder_factory(args, pretrained=True):
    ...
    """
    # build model
    model = M_Fusion(width=1, in_dim=args.in_dim, feat_dim=args.feat_dim).to(args.device)
    if pretrained:
        # load pre-trained model
        print('==> loading pre-trained model')
        ckpt = torch.load('./save/Mfusion/twins_epoch_699_0.45754.pth')
...
def main():
    ...
    # predictor network
    classifier = MLP(args.feat_dim, n_classes).to(args.device)
    args.visual = True
    if args.valid:
        try:
