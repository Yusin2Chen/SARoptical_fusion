...
def parse_option():
    ...
    parser.add_argument('--save', type=str, default='./mlp_DCCA1000', help='path to save linear classifier')
    parser.add_argument('--save_freq', type=int, default=50, help='number of training epochs')
    parser.add_argument('--preview_dir', type=str, default='./preview_DCCA', help='path to preview dir (default: no previews)')
...
def cmc_encoder_factory(args, pretrained=True):
    ...
    """
    # build model
    model = DCCA(n_inputs=args.n_inputs)
    if pretrained:
        # load pre-trained model
        print('==> loading pre-trained model')
        ckpt = torch.load('./save/DCCA/DCCA_epoch_699_-8.302835464477539.pth')
...
def train(epoch, train_loader, pretra_network, classifier, criterion, optimizer, scheduler, args):
    """
    one epoch training
    """
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if not args.valid:
            ...
            train(epoch, train_loader, pretra_network, classifier, criterion, optimizer, scheduler, args)
            if epoch % 50 == 0:
